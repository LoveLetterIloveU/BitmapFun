...
public class ImageSearchUtil
{
    ...
    public ImageDataSet<MediaInfo> getImagesFromSDCard(boolean getThumb)
    {
        ImageDataSet<MediaInfo> dataSet = new ImageDataSet<MediaInfo>();
    ...
    {
        m_imgThumbOptions.inSampleSize = BitmapUtil2.computeSampleSize(
                m_imgThumbOptions,
                getThumbMinSize(),
    ...
    public boolean getImageThumbnail(MediaInfo outImageInfo)
    {
        if (null != outImageInfo && null != m_contentResolver)
        {
            try
            {
                ...
                if (null == thumb)
                {
                    thumb = BitmapUtil2.makeBitmap(
                            getThumbMinSize(),
                            getThumbMaxNumOfPixels(),
                            outImageInfo.getImageUri(),
                            m_contentResolver,
                            false);
                }
                ...
                if (null == thumb)
                {
                    outImageInfo.setErrorDrawable(m_context);
                }
                else
                {
    ...
    {
        boolean retVal = Environment.getExternalStorageState().equals(
